...
func main() {
	...
	if *toc == true {
		mdwiki.Options["toc"] = "true"
	}
	if *noCache == true {
		mdwiki.Options["no-cache"] = "true"
	}
	if *silent == true {
		mdwiki.Options["silent"] = "true"
	}
	if *pandocArgs != "" {
		mdwiki.Options["pandoc-args"] = *pandocArgs
	}
	var err error
	if *root != "" {
		absRoot, err = filepath.Abs(*root)
		if err != nil {
			log.Fatal("absroot error")
		}
